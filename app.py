...
# Try to import modules, with fallback functions if they don't exist
try:
    ...
    class WeatherAPI:
        def get_weather(self, city, units):
            ...
            return {"name": city, "main": {"temp": 20, "feels_like": 18, "humidity": 60, "pressure": 1013, "temp_min": 15, "temp_max": 25}, 
                   "weather": [{"main": "Clear", "description": "clear sky", "icon": "01d"}], 
                   "wind": {"speed": 5}, "sys": {"country": "XX", "sunrise": 1234567890, "sunset": 1234567890}, "timezone": 0}
        
        def get_forecast(self, city, days, units):
            return {"city": {"name": city}, "list": []}
        
        def search_cities(self, query, limit):
            return []
        
        def get_weather_by_coordinates(self, lat, lon, units):
            return self.get_weather("Test City", units)
        
        def get_multiple_cities_weather(self, cities, units):
            return {"successful": {}, "errors": {}, "successful_count": 0, "total_requested": len(cities)}
...
try:
    ...
    def format_temperature(temp, units):
        return f"{temp:.1f}°{'C' if units == 'metric' else 'F'}"
    ...
    def format_wind_speed(speed, units):
        return f"{speed:.1f} {'m/s' if units == 'metric' else 'mph'}"
    ...
    def format_time(timestamp, offset=0):
        dt = datetime.fromtimestamp(timestamp + offset)
...
def main():
            ...
            if not validate_city_name(city):
                ...
                try:
                    with st.spinner(f"🔍 Getting weather data for {city}..."):
                        # Simple loading without progress bar to avoid compatibility issues
    ...
    """, unsafe_allow_html=True), city))
    
    def create_weather_summary(weather_data):
        return f"Weather summary for {weather_data.get('name', 'Unknown')}"
    
    def get_weather_advice(weather_data):
        return "Check the weather conditions before going out!"
    
    def color_temp_by_range(temp):
        if temp < 0: return "#3498db"
        elif temp < 10: return "#74b9ff"
        elif temp < 20: return "#00b894"
        elif temp < 30: return "#fdcb6e"
        else: return "#e17055"
